        ...
        return ListView(
          ...
          children: [
            GlassCard(
              child: Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(auth.userName, overflow: TextOverflow.ellipsis, maxLines: 1,
            ...
            _SettingsSection(title: 'Security', tiles: [
              const _SettingsTile(icon: Icons.lock_outline,     label: 'Two-Factor Auth'),
              const _SettingsTile(icon: Icons.key_outlined,     label: 'Change Password'),
              const _SettingsTile(icon: Icons.devices_outlined, label: 'Connected Devices'),
            ]),
            const SizedBox(height: 12),
            _SettingsSection(title: 'Storage', tiles: [
              const _SettingsTile(icon: Icons.cloud_outlined,          label: 'Cloud Storage',    trailing: Text('4.2 GB / 15 GB', style: TextStyle(color: AppTheme.textSec, fontSize: 12))),
              const _SettingsTile(icon: Icons.video_settings_outlined, label: 'Video Quality',    trailing: Text('1080p',          style: TextStyle(color: AppTheme.primary, fontSize: 12))),
              const _SettingsTile(icon: Icons.history_outlined,        label: 'Retention Period', trailing: Text('30 days',        style: TextStyle(color: AppTheme.primary, fontSize: 12))),
            ]),
