...
class CamerasTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final camC = CameraController.to;
      return SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              _StatCard(label: 'Online',  value: '${camC.onlineCount}',  icon: Icons.wifi,           color: AppTheme.primary),
              const SizedBox(width: 12),
              _StatCard(label: 'Offline', value: '${camC.offlineCount}', icon: Icons.wifi_off,       color: AppTheme.textSec),
              const SizedBox(width: 12),
              _StatCard(label: 'People',  value: '${camC.totalPeople}',  icon: Icons.people_outline, color: AppTheme.info),
            ]),
            ...
            Row(
              ...
              children: [
                Text(
                  camC.cameras.isEmpty ? 'No Cameras Yet' : 'Your Cameras (${camC.cameras.length})',
            ...
            if (camC.cameras.isEmpty)
              ...
              GridView.builder(
                ...
                itemCount: camC.cameras.length,
                itemBuilder: (_, i) => CameraCard(camera: camC.cameras[i]),
              ),
          ],
        ),
      );
  ...
  Widget build(BuildContext context) => Center(
    child: Padding(
      ...
      child: Column(children: [
        ...
        ElevatedButton.icon(onPressed: onAdd, icon: const Icon(Icons.add, size: 18), label: const Text('Add Camera')),
      ]),
    ),
  );
}
...
class _AddCameraSheet extends StatefulWidget {
  ...
  @override State<_AddCameraSheet> createState() => _AddCameraSheetState();
}
...
class _AddCameraSheetState extends State<_AddCameraSheet> {
  ...
  void _submit() {
    ...
    CameraController.to.addCamera(name: _nameCtrl.text, location: _locationCtrl.text);
    Get.back();
  }
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Form(
      ...
      child: Column(
        ...
        children: [
          Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: AppTheme.surface2, borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 20),
          Row(children: [
            Container(width: 40, height: 40, decoration: BoxDecoration(color: AppTheme.primary.withOpacity(0.12), borderRadius: BorderRadius.circular(12)),
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(labelText: 'Camera Name *', hintText: 'e.g. Front Door, Living Room',
          ...
          TextFormField(
            ...
            style: const TextStyle(color: AppTheme.textPri),
            decoration: const InputDecoration(labelText: 'Location (optional)', hintText: 'e.g. Home • Entrance',
              prefixIcon: Icon(Icons.location_on_outlined, color: AppTheme.textSec, size: 20)),
            textCapitalization: TextCapitalization.words,
          ),
          const SizedBox(height: 28),
          ElevatedButton.icon(onPressed: _submit, icon: const Icon(Icons.add, size: 18), label: const Text('Add Camera')),
          const SizedBox(height: 8),
          TextButton(onPressed: () => Get.back(), child: const Text('Cancel', style: TextStyle(color: AppTheme.textSec))),
        ],
      ),
    ),
  );
          ...
          Padding(
            ...
            child: GlassCard(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: Text('${alertC.unreadCount} unread alerts', style: const TextStyle(color: AppTheme.textPri, fontWeight: FontWeight.w500))),
                TextButton(onPressed: alertC.markAllRead, child: const Text('Mark all read', style: TextStyle(color: AppTheme.primary, fontSize: 12))),
              ]),
            ),
          ),
        Expanded(
          child: ListView.separated(
            padding: const EdgeInsets.all(16),
            itemCount: alertC.alerts.length,
            separatorBuilder: (_, __) => const SizedBox(height: 8),
            itemBuilder: (_, i) {
              final a = alertC.alerts[i];
              return GestureDetector(
                onTap: () => alertC.markRead(a.id),
                child: GlassCard(
                  ...
                  child: Row(children: [
                    ...
                    Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                      ...
                      if (!a.isRead) Container(width: 8, height: 8, margin: const EdgeInsets.only(top: 4),
...
class AnalyticsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final camC       = CameraController.to;
      final withPeople = camC.cameras.where((c) => c.peopleCount > 0).toList();
      return SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            ...withPeople.map((cam) => Padding(
              ...
              child: GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Expanded(child: Text(cam.name, overflow: TextOverflow.ellipsis, maxLines: 1,
